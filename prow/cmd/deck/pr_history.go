...
// parsePullURL parses PR history URLs according to two possible formats:
//
// 1. <host>/pr-history/<org>/<repo>/<pr number>
//
// 2. <host>/pr-history?org=<org>&repo=<repo>&pr=<pr number>
func parsePullURL(u *url.URL) (org, repo string, pr int, err error) {
	key := strings.TrimPrefix(u.Path, "/pr-history/")
