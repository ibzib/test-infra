...
import {Job, JobState, JobType, Refs} from "../api/prow";
    ...
    for (let i = 0; i < allBuilds.length; i++) {
        ...
        if (key !== lastKey) {
            ...
            if (build.type === "periodic") {
                ...
                let repo_link = build.refs.repo_link;
                if (!repo_link) {
                    repo_link = "https://github.com/" + build.refs.repo;
                }
...
function batchRevisionCell(build: Job): HTMLTableDataCellElement {
    ...
    for (let i = 1; i < build.refs.pulls.length; i++) {
        ...
        l.href = build.refs.pulls[i].link;
        l.text = String(build.refs.pulls[i].number);
        c.appendChild(document.createTextNode("#"));
        c.appendChild(l);
    }
    return c;
